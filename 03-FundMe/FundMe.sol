...
contract FundMe {
    ...
    //mapping(address userAddress => uint256 amountFunded) public addressToAmountFunded;
    ...
    modifier onlyOwner() {
        ...
        //Hint: Notice how we replaced the require with an if statement and then call the defined custom error. this helps us to save gas ie Using Custom error helps us to save gas
    ...
    function withdraw() public onlyOwner {
        for (
            ...
            // Execute everything inside the loop here for the duration specified above ie  /*Starting index, Ending index, Step amount*/
        ...
        /**
         * Hint: call allows us to call different function. The call() returns two variables, 1: a boolean that shows success and
         * 2: bytes of the data returned from the function call
         *
         * here were replace the funtion call with empty string because we are not making any call ie (""), thus if you are making function call, you simply pass in the function inside the call() and then get the returned data as bytes.
         *
    ...
    // Hint: With the fallback() and receive() defined here if someone accidentally send money to this contract without calling the fund(), then it will automatically  forward it to the fund funtion
    ...
    // Hint: Recieve is only triggered when the calldata is empty... Thus if it is empty and there is a recieve funtion, it will be stored in the receive funtion, but if is empty and there is no receieve funtion then it will be stored in the fallback
